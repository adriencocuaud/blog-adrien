...
// l'algorithme après le temps d'inspection réglé (contrôle −/+ du panneau d'options),
// en animant sa résolution sur le cube à une vitesse déduite de ce même temps.
// Le groupe tiré (OLL / PLL) suit le switch de la page, géré par js/algorithme-switch.js.
(function () {
    // Temps d'inspection avant la réponse, réglable par pas d'une seconde
    var REVEAL_DELAY_DEFAULT = 4000;
    var REVEAL_DELAY_MIN = 1000;
    var REVEAL_DELAY_MAX = 15000;
    var REVEAL_DELAY_STEP = 1000;
    // Tempo de l'animation déduit du temps d'inspection : une valeur par seconde,
    // décroissance non linéaire (marches larges en haut, resserrées en bas).
    // Repères voulus : x5 à 1 s, x3 à 5 s, x1 à partir de 10 s.
    var TEMPO_BY_SECOND = {
        1: 5, 2: 4.5, 3: 4, 4: 3.5, 5: 3,
        6: 2.5, 7: 2.1, 8: 1.7, 9: 1.3, 10: 1
    };
    var TEMPO_MIN = 1; // au-delà de la table (≥ 10 s) : vitesse réelle
    var NEXT_PAUSE = 200; // pause après la durée de l'algo, avant le cas suivant
    var AUF_PAUSE = 100; // pause entre la rotation AUF jouée et l'algo de résolution
    var AUF_FALLBACK = 600; // repli si la durée de la rotation AUF n'est pas lisible
    ...
    // Groupe tiré : celui du switch de la page (js/algorithme-switch.js gère son
    // affichage, on n'écoute ici que le changement pour relancer un cycle).
    var groupSwitchEl = document.getElementById('group-switch');
    var groupButtons = groupSwitchEl
        ? groupSwitchEl.querySelectorAll('.group-switch-option')
        : [];
    ...
    // Pas de switch sur la page : on tire dans le premier groupe déclaré
    if (!activeGroup) activeGroup = Object.keys(entriesByGroup)[0] || null;
    ...
    // Phase 2 du reveal : l'algo seul, depuis l'état atteint par la rotation AUF.
    // Setup = orientation de base : l'ancre en fin d'alg fait donc partir de
    // « orientation + algo inversé », exactement l'état laissé par la phase 1.
    function playAlgo3d(item, token) {
        if (token !== roundToken) return; // le cas a changé pendant la pause
        try {
            player3d.setAttribute('experimental-setup-alg', TOP_SETUPS[topColor]);
            player3d.alg = algoText(item.formula);
        } catch (e) { /* notation refusée : le cube reste sur l'état précédent */ }
        play3d();
        scheduleNext();
    }

    // Reveal : sans AUF, l'algo est joué d'un bloc. Avec AUF, la rotation est jouée
    // seule, puis AUF_PAUSE, puis l'algo. Le setup de la phase 1 (orientation + algo
    // inversé, notation ( ... )' de cubing.js) donne le même état de départ que celui
    // affiché pendant l'inspection : le raccord entre les deux phases est sans saut.
    function playReveal(item) {
        if (!currentAuf || !player3dReady() || !item) {
            play3d();
            scheduleNext();
            return;
        }
        ...
        function startPhase2() {
            ...
            phase2Done = true;
            playAlgo3d(item, token);
        }
        try {
            player3d.setAttribute('experimental-setup-alg', TOP_SETUPS[topColor] + ' (' + algoText(item.formula) + ")'");
            player3d.alg = currentAuf;
        } catch (e) {
            // Notation refusée : on retombe sur le cas posé tel quel, joué d'un bloc
            setup3dCase(item);
            play3d();
            scheduleNext();
            return;
        ...
        play3d();
        // Repli armé tout de suite : si la lecture du modèle échoue ou traîne,
        // l'algo part quand même
        aufTimer = setTimeout(startPhase2, AUF_FALLBACK + AUF_PAUSE);
        if (typeof Promise === 'undefined') return;
        try {
            animation3dDuration().then(function (duration) {
                if (token !== roundToken || phase2Done) return;
                clearTimeout(aufTimer);
                aufTimer = setTimeout(startPhase2, Math.min(duration, ANIM_MAX_DELAY) + AUF_PAUSE);
            })['catch'](function () { /* repli déjà armé */ });
        } catch (e) { /* repli déjà armé */ }
    }

    // --- Réglage : temps d'inspection (et tempo d'animation qui en découle) ---
    // Le réglage étant au pas de la seconde, le tempo se lit dans la table
    function tempoFor(delayMs) {
        return TEMPO_BY_SECOND[Math.round(delayMs / 1000)] || TEMPO_MIN;
    }
    ...
    function applyTiming() {
        ...
        if (player3d) player3d.setAttribute('tempo-scale', tempoFor(revealDelay).toFixed(2));
    }
    ...
    [timerMinusBtn, timerPlusBtn].forEach(function (btn) {
        if (!btn) return;
        var delta = btn === timerMinusBtn ? -REVEAL_DELAY_STEP : REVEAL_DELAY_STEP;
        btn.addEventListener('click', function (event) {
            event.stopPropagation();
            adjustRevealDelay(delta);
        });
    ...
    // --- Panneau d'options (vitesse, vue, cas), affiché à la place du cube ---
    ...
    // Changement de groupe au switch de la page : nouveau cycle complet.
    // L'affichage du switch et des sections est fait par js/algorithme-switch.js.
    groupButtons.forEach(function (btn) {
    ...
    if (window.customElements && customElements.whenDefined) {
        try {
            customElements.whenDefined('twisty-player').then(function () {
                applyTiming(); // le tempo doit être posé avant la lecture de la durée
                setup3dCase(current);
